...
def calculations(input)
	...
	if only_numbers(input) == true
		...
		if dollar_counter > 0
			total[:dollar] = dollar_counter
		end

		if quarter_counter > 0
			total[:quarter] = quarter_counter
		end

		if dime_counter > 0
			total[:dime] = dime_counter
		end

		if nickel_counter > 0
			total[:nickel] = nickel_counter
		end

		if penny_counter > 0
			total[:penny] = penny_counter
		end
